...
if [ -z "$PROFILE" ]; then
  echo "用法: $0 <deepseek|deepseek-tc|deepseek-anthropic|layer2-search|hybrid-ds|deepqwen|direct [qwen|deepseek]|hybrid|hybrid-toolfree|layer1|layer2|layer3>"
  echo ""
  ...
  echo "  deepqwen — DeepSeek+Qwen 组合，mini-router 真透传"
...
case "$PROFILE" in
  direct)
    ...
    ;;
  hybrid|hybrid-toolfree|layer1|layer2|layer3|deepseek|deepseek-tc|deepseek-anthropic|layer2-search|hybrid-ds|deepqwen) ;;
...
case "$PROFILE" in
  ...
  deepqwen)
    mkdir -p "$CCR_CONFIG_DIR"
    TMPCFG=$(mktemp)
    resolve_secret "$PROFILES_DIR/deepqwen/ccr-config.json" "$TMPCFG"
    if [ -f "$CCR_CONFIG_DIR/config.json" ]; then
      python3 "$MERGE" ccr "$CCR_CONFIG_DIR/config.json" "$TMPCFG"
    else
      cp "$TMPCFG" "$CCR_CONFIG_DIR/config.json"
    fi
    cp "$PROFILES_DIR/deepqwen/custom-router.js" "$CCR_CONFIG_DIR/custom-router.js"
    rm -f "$TMPCFG"
...
case "$PROFILE.$DIRECT_PROVIDER" in
  ...
  deepqwen)              echo "     DeepSeek+Qwen 组合，mini-router 真透传" ;;
